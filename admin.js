...
(() => {
  ...
  async function orders() {
    ...
    try{const data=await api('/api/admin/orders?'+orderQuery+'&page='+page);if(generation!==orderGeneration)return;
      $('orders-body').innerHTML=data.orders.map(o=>`<tr><td>${esc(o.id)}<small>${esc(o.created_at)}</small></td><td>${esc(o.player_username)}<small>ID ${esc(o.user_id??'已刪除')} · ${esc(o.player_email)}</small></td><td>${esc(o.product_name)}<small>${esc(o.product_id)}</small></td><td>${o.quantity}</td><td>${o.total.toLocaleString()} ${o.currency==='COIN'?'金幣':esc(o.currency)}<small>單價 ${o.unit_price.toLocaleString()}</small></td><td>${badge(o.status)}</td></tr>`).join('');
