...
export function catalogHandlers(kind) {
  ...
  function save(create) {
    return async ({request,env,data}) => {
      ...
      let body;
      try { const raw = await request.text(); if(raw.length > 100000) throw new Error(); body=JSON.parse(raw); } catch { return json({ok:false,error:'invalid-body'},{status:400}); }
      ...
      const keys = kind === 'card' ? ['name','subtitle','image','collection','mana','attack','health','abilities','translations','nameLayout','rulesLayout'] : ['name','cardIds','coinPrice'];
      const clean = Object.fromEntries(keys.filter(k=>body[k] !== undefined).map(k=>[k,body[k]]));
      if(kind === 'card' && clean.translations?.['zh-TW']) clean.translations['zh-TW']={name:clean.name,subtitle:clean.subtitle,abilities:clean.abilities};
