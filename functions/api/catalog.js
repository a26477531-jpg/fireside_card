...
export async function onRequestGet({env}) {
  ...
  return json({ok:true,source:'database',cards:cards.results.map(row=>{const {translationMeta,...card}=decode(row);return card;}),products:products.results.map(row=>{const {translationMeta,...product}=decode(row);return product;}).filter(p=>p.cardIds.every(id=>ids.has(id)))});
}
